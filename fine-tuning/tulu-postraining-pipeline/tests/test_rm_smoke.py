...
phase 2: code only. phase 5 gpu run:
  ...
  RUN_RM_SMOKE=1 PYTHONPATH=src python -m pytest tests/test_rm_smoke.py -s
"""
...
from pipeline.reward_model import RM_NUM_TRAIN_EPOCHS, build_rm_config, resolve_sft_checkpoint
...
def _smoke_cfg() -> dict:
    with (ROOT / "configs" / "rm.yaml").open() as f:
