"""unit tests for analysis cli wiring (no matplotlib required for these)."""
...
from pipeline.analysis.cli import (
    _merge_stage_map,
    _parse_stage_pairs,
    _selected_modes,
    main,
...
def test_parse_args_modes() -> None:
    args = parse_args(["--attribution", "--skills-json", "skills.json"])
    assert _selected_modes(args) == ["attribution"]

    args = parse_args(
        ["--verdict", "--dpo-summary", "d.json", "--ppo-summary", "p.json"]
    )
    assert _selected_modes(args) == ["verdict"]
    assert args.dpo_name == "dpo"
...
def test_main_attribution_missing_skills() -> None:
    assert main(["--attribution"]) == 2
