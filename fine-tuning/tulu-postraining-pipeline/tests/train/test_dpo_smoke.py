...
phase 2: code only. phase 5 gpu run:
  ...
  RUN_DPO_SMOKE=1 DPO_SMOKE_SFT_CHECKPOINT=<sft> \\
    PYTHONPATH=src python -m pytest tests/train/test_dpo_smoke.py -s
"""
...
from trainers.dpo import build_dpo_config, format_dpo_task, resolve_beta, resolve_sft_checkpoint
...
def _smoke_cfg() -> dict:
    with (REPO_ROOT / "configs" / "dpo.yaml").open() as f:
